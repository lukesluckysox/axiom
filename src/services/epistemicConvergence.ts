...
export function buildConvergencePayload(
  ...
  const liminalClean = distillText((liminal.title || liminal.summary || 'A reflective observation').slice(0, 120).trim());
  const parallaxClean = distillText((parallax.title || parallax.summary || 'An observed pattern').slice(0, 120).trim());
